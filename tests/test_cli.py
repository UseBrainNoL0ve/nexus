...
class DiagnoseCliTests(unittest.TestCase):
    ...
    def test_diagnose_json_is_read_only_and_structured(self):
        ...
        incident = Incident(
            id="incident:resource:memory-pressure",
            severity="warning",
            category="resource",
            title="Memory pressure is persistent",
            findings=(finding.id,),
            evidence=(finding.evidence,),
            summary="1 related resource finding(s) require review.",
        )
        ...
        output = io.StringIO()
        with patch("nexus.cli.collect_diagnostics", return_value=([finding], [])), patch(
            "nexus.cli.build_incidents", return_value=[incident]
        ), patch("nexus.cli.build_remediation_plan", return_value=[step]), contextlib.redirect_stdout(output):
        ...
        self.assertEqual(payload["remediation_steps"][0]["action"], "Inspect memory-heavy processes")
        self.assertTrue(payload["read_only"])
